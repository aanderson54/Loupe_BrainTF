...
top<-GO_ALL[which(GO_ALL$encodeLabel!="dELS"),] %>% group_by(encodeLabel) %>% top_n(n=7, wt=-p_adjust)
top<-top[-c(2),]
...
pdf("GO_enrichment_dotPlot_ENCODE_annot2.pdf", width=6, height=7)
ggplot(GO_top, aes(x=encodeLabel,y=description, color=-log10(p_adjust), size=log2(fold_enrichment)))+geom_point()+theme_bw()+scale_color_gradient2(low="blue",mid="purple",high="red", midpoint=2, na.value="red", limits=c(0,8))+scale_radius(range=c(1,7), breaks=c(-1.5,-1,0,1,1.5), labels=c(-1.5,-1,0,1,1.5))+scale_y_discrete(labels = function(x) str_wrap(x, width = 40))
dev.off()
...
union_files<-as.list(readLines("BrainTF/all_union_files.csv"))
m=sapply(strsplit(unlist(union_files),"[/]"),"[[",8)
m2=sapply(strsplit(m,".", fixed=T),"[[",1)
m2<-gsub("union_","", m2)
union_list=lapply(union_files,function(x){
...
unlist_files<-as.list(readLines("BrainTF/all_unlist_files.csv"))
m=sapply(strsplit(unlist(unlist_files),"[/]"),"[[",8)
m2=sapply(strsplit(m,".", fixed=T),"[[",1)
m2<-gsub("unlist_","", m2)
unlist_list=lapply(unlist_files,function(x){
  tmp<-GRanges(read.csv(x))
...
p2<-ggplot(p_unlist[which(p_unlist$PKNOX1<=1),], aes(x=PKNOX1, fill=group,y=ct))+geom_boxplot(alpha=0.85)+theme_classic()+ylab("cell type")+xlab("Proportion of TFs co-binding")+theme(axis.text.y=element_blank())+scale_fill_manual(limits=c("CB","DLPFC","FP","OL","DLPFC_OLIG","FP_OLIG","OL_OLIG","DLPFC_NEUN","FP_NEUN","OL_NEUN"), values=c("darkgoldenrod2","gold2","gold1","goldenrod1","green3","darkgreen","forestgreen","cornflowerblue","dodgerblue","darkblue"), name="")
...
write.csv(tmp.m, "BrainTF/Motif_proportion.csv")
